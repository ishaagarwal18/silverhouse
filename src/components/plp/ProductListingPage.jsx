...
export default function ProductListingPage({
  ...
  const filteredProducts = useMemo(() => {
    ...
    return result;
  }, [backendItems, products, selectedCategory, selectedSubcategory, selectedPurity, selectedRecipient, maxPrice, inStockOnly, sortBy]);
    ...
    <div className="bg-[#FAFAFA] min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-8">
        ...
        <div className="flex gap-8 items-start">
          ...
          <aside className={`w-64 bg-white p-6 rounded-2xl border border-silver-200 shadow-xs space-y-6 shrink-0 ${isMobileFilterOpen ? 'fixed inset-y-0 left-0 z-50 overflow-y-auto w-80 shadow-2xl block' : 'hidden lg:block'
            }`}>
            ...
            <div>
              ...
              <div className="space-y-1 text-xs">
                <button
                  onClick={() => handleCategorySelect('all')}
                  className={`w-full text-left py-1.5 px-2 rounded-md font-medium transition-colors ${selectedCategory === 'all' ? 'bg-[#1A1A1A] text-white font-bold' : 'text-silver-700 hover:bg-silver-100'
                    }`}
                >
                  All Categories
                </button>
                {categoryList.map((cat) => (
                  <button
                    ...
                    className={`w-full text-left py-1.5 px-2 rounded-md font-medium transition-colors ${selectedCategory === cat.id ? 'bg-[#1A1A1A] text-white font-bold' : 'text-silver-700 hover:bg-silver-100'
                      }`}
                  >
              ...
              <div className="space-y-1 text-xs">
                {['all', 'Puja', 'Baby', 'Kids', 'Gifting', 'Investment'].map((tag) => (
                  <button
                    ...
                    className={`mr-1 mb-1 px-2.5 py-1 rounded-full text-[11px] font-semibold transition-colors ${selectedRecipient === tag
